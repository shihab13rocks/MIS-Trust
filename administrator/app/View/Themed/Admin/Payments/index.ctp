				...
				<tbody>
					<?php foreach ($payments as $payment): ?>
							<tr>
								...
								<td><?php echo $this->Generic->viewStatus(h($payment['Payment']['status'])); ?>&nbsp;</td>
								...
								<td class="actions">
									<div class="btn-group">
										<?php if(Configure::read('Action.view')): ?>
											<?php echo $this->Html->linkIcon(
												'', 
												array('action' => 'view', $payment['Payment']['id']),
												array('class'=>'btn btn-warning','icon'=>'fa fa-eye')); 
											?>
										<?php endif; ?>
										
										<?php if(Configure::read('Action.edit') || (AuthComponent::user('id') == $payment['Payment']['id'])): 
											
												$actionMethod = (AuthComponent::user('id') == $payment['Payment']['id'])? 'profile':'edit';
										?>
											<?php echo $this->Html->linkIcon(
												'', 
												array('action' => $actionMethod, $payment['Payment']['id']),
												array('class'=>'btn btn-warning','icon'=>'fa fa-pencil-square-o')); 
											?>
										<?php endif; ?>	
										
										<?php if(Configure::read('Action.delete')): ?>
											<?php if (AuthComponent::user('id')!=$payment['Payment']['id']): ?>
	
												<?php echo $this->Form->postLink(
													'', 
													array('action' => 'delete', $payment['Payment']['id']), 
													array('class'=>'btn btn-danger delete','icon'=>'fa fa-bitbucket-square'),
													__('Are you sure you want to delete "%s"?', $payment['Payment']['title']));								
												?>
											<?php endif; ?>
										<?php endif; ?>
									</div>
